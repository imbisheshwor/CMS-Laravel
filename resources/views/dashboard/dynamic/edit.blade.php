            ...
            <div class="flex-none w-full max-w-full px-3">
                <div
                    ...
                    <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                        <h6 class="">Edit Entiry of <b>{{$cpt->name}}</b></h6>
                    ...
                    <div class="flex-auto px-0 pt-0 pb-2">
                        <div class="p-0 overflow-x-auto mt-10 pl-10">
                            ...
                            <form action="{{ route('store.update', ['custom_post_type_slug' => $slug, 'key' => $key]) }}"
                                method="POST">
                                @csrf
                                ...
                                <div class="mb-6">
                                        <label for="{{$item->entity->slug}}"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{$item->entity->name}}</label>
                                        <input type="{{$item->entity->type}}" name="values[{{$item->id}}]" value="{{$item->value}}"
